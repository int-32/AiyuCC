---
description: 自动编码执行。读取计划目录中的模块文档，找到未完成任务，逐个编码、测试、提交、更新状态。
---

# /coding - 自动编码执行

读取计划目录中各模块文档的未完成任务，自动执行编码 → 测试 → 提交 → 更新状态的循环。
...
支持:
- 计划目录路径，如 `/coding .claude/plans/user-auth/`
- 计划名称，如 `/coding user-auth`（自动解析为 `.claude/plans/user-auth/`）
...
- 读取计划目录下的 `_overview.md`，获取模块文档索引和全局任务清单
- 按索引依次读取各模块文档（`<module-name>.md`）
- 从各模块文档中解析 `- [ ]` 未完成任务，汇总为全局任务列表
- 跳过已完成的 `- [x]` 任务
- 如果没有未完成任务，提示用户并退出
...
- 对应模块文档: `- [ ]` → `- [x]`（用 Edit 工具更新模块文件中的任务清单）
- `_overview.md` 全局任务清单: 同步 `- [ ]` → `- [x]`
...
| 文件冲突 | 暂停，展示冲突内容，询问用户 |
| 计划目录不存在 | 提示运行 `/plan` 先创建计划 |
| 模块文档缺失 | 提示 `_overview.md` 索引与实际文件不匹配，需检查 |
...
```
已加载计划: 用户认证系统
目录: .claude/plans/user-auth/
模块: 3 个 (module-a, module-b, module-c)
未完成任务: 5/8

[1/5] [module-a] Wave1: 创建 User 模型 (src/models/user.py)
  编码中... 完成
  测试中... 3/3 通过
  提交中... feat(auth): add User model with password hashing
  已更新: module-a.md + _overview.md

[2/5] [module-b] Wave2: 创建认证路由 (src/routes/auth.py)
  编码中... 完成
  测试中... 5/5 通过
  提交中... feat(auth): add login and register endpoints
  已更新: module-b.md + _overview.md
